...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            ...
            data = {
                'success': False,
                'message': 'This is phone number already exists'
            }
            raise ValidationError(data)
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate_username(self, username):
        ...
        if username.isdigit():
            raise ValidationError(
                {'message': 'This username is intirely numeric'}
            )
        return username
    ...
    def validate_first_name(self, first_name):
        if len(first_name) < 5 or len(first_name) > 30:
            raise ValidationError(
                {'message': 'Firstname must be between 5 and 30 characters long'}
            )
        if first_name.isdigit():
            raise ValidationError(
                {'message': 'This firstname is intirely numeric'}
            )
        return first_name
    ...
    def validate_last_name(self, last_name):
        if len(last_name) < 5 or len(last_name) > 30:
            raise ValidationError(
                {'message': 'Lastname must be between 5 and 30 characters long'}
            )
        if last_name.isdigit():
            raise ValidationError(
                {'message': 'This lastname is intirely numeric'}
            )
        return last_name
...
class ChangeUserPhotoSerializer(serializers.Serializer):
    photo = serializers.IntegerField(validators=[FileExtensionValidator(allowed_extensions=['jpg', 'jpeg', 'png', 'heic', 'heif'])])
